...
import raf.aleksabuncic.repository.specification.VeterinarianSpecifications;
...
public class VeterinarianServiceImplementation implements VeterinarianService {
    ...
    @Transactional(readOnly = true)
    @Override
    public Iterable<VeterinarianDto> findAllVeterinarians(VeterinarianSearchDto veterinarianSearchDto, Pageable pageable) {
        log.info("Finding all veterinarians with filters: {} and pagination: {}", veterinarianSearchDto, pageable);

        Specification<Veterinarian> specification = VeterinarianSpecifications.build(veterinarianSearchDto);

        return veterinarianRepository.findAll(specification, pageable)
                .map(veterinarianMapper::veterinarianToVeterinarianDto);
    }
